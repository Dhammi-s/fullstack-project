...
export default function WorkerAssignments() {
  ...
  const handleUpdate = async (id, status) => {
    try { await ordersApi.updateStatus(id, status); toast.success('Updated!'); fetch(); }
            ...
            <div key={order.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between flex-wrap gap-3">
                <div className="flex-1">
                  ...
                  <h3 className="font-semibold text-gray-800 mb-1">{order.serviceName || 'Product Order'}</h3>
                  <div className="flex flex-wrap gap-4 text-sm text-gray-500 mt-2">
                    ...
                    {order.scheduledAt && <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" /> {new Date(order.scheduledAt).toLocaleDateString()}</span>}
                    <span className="flex items-center gap-1 text-green-600 font-semibold"><DollarSign className="w-3.5 h-3.5" /> ${order.totalAmount?.toFixed(2)}</span>
                  </div>
                </div>
                <div className="flex gap-2">
                  {order.status === 'Pending' && (
                    <button onClick={() => handleUpdate(order.id, 'InProgress')} className="btn-primary text-xs py-2">Start Job</button>
                  )}
                  {order.status === 'InProgress' && (
                    <button onClick={() => handleUpdate(order.id, 'Completed')} className="btn-success text-xs py-2">Mark Complete</button>
                  )}
